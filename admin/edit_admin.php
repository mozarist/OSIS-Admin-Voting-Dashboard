...
//eksekusi
$go = mysqli_query($koneksi, $data_admin_per_id);
...
if (isset($_POST['go'])) {
    ...
    //query         
    $sql = "UPDATE `admin` set `username`='$username', `nama_lengkap`='$nama_lengkap', `password`='$password' where id_admin='$id_admin'";
...
<form action="" method="POST" class="w-full space-y-5 self-center p-5">
    <h2 class="font-semibold text-3xl text-center">Edit Data Administrator</h2>
        ...
        <div class="flex flex-col lg:flex-row gap-4">
            <div class="flex-2 flex flex-col gap-3 order-2 lg:order-1">
                <div class="flex flex-col gap-1 w-full">
                    ...
                    <input type="text" name="username" value="<?=$ambil_data['username']?>" class="p-2 border border-gray-300 rounded-xl">
                </div>
                <div class="flex flex-col gap-1 w-full">
                    <label class="font-semibold text-gray-600">Nama Lengkap</label>
                    <input type="text" name="nama_lengkap" value="<?=$ambil_data['nama_lengkap']?>" class="p-2 border border-gray-300 rounded-xl">
                </div>
                <div class="flex flex-col gap-1 w-full">
                    <label class="font-semibold text-gray-600">Password</label>
                    <div class="relative w-full">
                        <input type="password" id="password" name="password" value="<?=$ambil_data['password']?>"
